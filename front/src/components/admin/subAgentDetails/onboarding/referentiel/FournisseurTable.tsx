...
import type { Supplier } from '../../../../../lib/onboarding/types';
import { supplierApi } from '../../../../../lib/onboarding/supplier';
import { Pagination } from './Pagination';

const PAGE_SIZE = 10;
const C = { color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' };
...
const empty: Partial<Supplier> = { companyName: '', firstName: '', lastName: '', email: '', phone: '', address: '', city: '', country: 'Maroc', category: '', website: '', ice: '', iban: '', status: 'active', notes: '' };

const FormPopup: React.FC<{ title: string; initial: Partial<Supplier>; onSave: (d: Partial<Supplier>) => void; onClose: () => void; loading?: boolean }> = ({ title, initial, onSave, onClose, loading }) => {
  const [form, setForm] = useState<Partial<Supplier>>(initial);
  const set = (k: keyof Supplier) => (v: string) => setForm(p => ({ ...p, [k]: v }));
      ...
      <div style={{ width: '68vw', maxWidth: 820, maxHeight: '90vh', background: 'var(--bg)', borderRadius: 16, boxShadow: '0 32px 80px rgba(0,0,0,0.35)', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'onb-popup 0.2s ease-out' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 20px', marginBottom: 18 }}>
            ...
            <Field label="Catégorie" value={form.category || ''} onChange={set('category')} />
          ...
          <p style={sectionSt}>Bancaire</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 20px', marginBottom: 18 }}>
            <Field label="Banque" value={form.bankName || ''} onChange={set('bankName')} />
...
export const FournisseurTable: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await supplierApi.getAll({ page, limit: PAGE_SIZE, search: search || undefined, status: statusF || undefined, country: countryF || undefined, category: catF || undefined });
      setData(res.data); setTotal(res.total); setTotalPages(res.totalPages);
    } catch (e: any) { setError(e.message || 'Erreur'); } finally { setLoading(false); }
  }, [page, search, statusF, countryF, catF]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { setPage(0); }, [search, statusF, countryF, catF]);

  const handleCreate = async (dto: Partial<Supplier>) => { setSaving(true); try { await supplierApi.create(dto); setCreating(false); load(); } catch (e: any) { alert(e.message); } finally { setSaving(false); } };
  const handleEdit   = async (dto: Partial<Supplier>) => { if (!editing) return; setSaving(true); try { await supplierApi.update(editing.id, dto); setEditing(null); load(); } catch (e: any) { alert(e.message); } finally { setSaving(false); } };
  const handleDelete = async (id: string) => { if (!confirm('Supprimer ce fournisseur ?')) return; try { await supplierApi.remove(id); load(); } catch (e: any) { alert(e.message); } };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border)', background: 'var(--bg2)', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <select value={catF} onChange={e => setCatF(e.target.value)} style={selSt}>
          <option value="">Toutes catégories</option>
          {[...new Set(data.map(d => d.category).filter(Boolean))].map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        {total > 0 && <span style={{ fontSize: '0.7rem', color: C.color, fontWeight: 600 }}>{total} fournisseur{total > 1 ? 's' : ''}</span>}
        <button onClick={() => setCreating(true)} style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, border: `1.5px solid ${C.color}`, background: C.bg, color: C.color, cursor: 'pointer', fontSize: '0.76rem', fontWeight: 700, flexShrink: 0, whiteSpace: 'nowrap' }}>
          <Icons.Plus style={{ width: 13, height: 13 }} /> Nouveau fournisseur
        </button>
      </div>
      ...
      <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto' }}>
        ...
        : data.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text3)' }}>
            ...
            <div style={{ fontSize: '0.85rem' }}>Aucun fournisseur trouvé</div>
          </div>
        ) : (
          <table style={{ width: '100%', minWidth: 860, borderCollapse: 'collapse', fontSize: '0.74rem' }}>
            <thead style={{ position: 'sticky', top: 0, zIndex: 2 }}>
              <tr style={{ background: 'var(--bg2)', borderBottom: '1px solid var(--border)' }}>
                {['Nom', 'Email', 'Téléphone', 'Pays / Ville', 'Catégorie', 'ICE', 'IBAN', 'Statut', ''].map(h => (
              ...
              {data.map((d, i) => {
                ...
                return (
                  <tr key={d.id} onClick={() => setDetail(d)}
                    ...
                    <td style={{ padding: '10px 11px' }}><span style={{ padding: '2px 7px', borderRadius: 4, fontSize: '0.63rem', background: 'var(--bg2)', color: 'var(--text2)', border: '1px solid var(--border)', whiteSpace: 'nowrap' }}>{d.category || '—'}</span></td>
                    <td style={{ padding: '10px 11px' }}><span style={{ fontFamily: 'monospace', fontSize: '0.7rem', color: 'var(--text2)' }}>{d.ice || '—'}</span></td>
                    <td style={{ padding: '10px 11px' }}><span style={{ fontFamily: 'monospace', fontSize: '0.68rem', color: 'var(--text2)' }}>{d.iban || '—'}</span></td>
      ...
      {detail   && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1500, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={e => e.target === e.currentTarget && setDetail(null)}>
          <div style={{ width: '58vw', maxWidth: 680, maxHeight: '86vh', background: 'var(--bg)', borderRadius: 16, boxShadow: '0 32px 80px rgba(0,0,0,0.35)', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'onb-popup 0.2s ease-out' }}>
            <div style={{ padding: '18px 22px', background: `linear-gradient(135deg,${C.bg},transparent)`, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexShrink: 0 }}>
              <div>
                ...
                <div style={{ fontSize: '0.72rem', color: 'var(--text2)', marginTop: 3 }}>{detail.category} {detail.city && `· ${detail.city}`} {detail.country && `, ${detail.country}`}</div>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '18px 22px' }}>
              {[
                ...
                { title: 'Légal', rows: [['ICE', detail.ice], ['RC', detail.rc], ['Tax ID', detail.taxId]] },
                { title: 'Bancaire', rows: [['Banque', detail.bankName], ['IBAN', detail.iban], ['SWIFT', detail.swift]] },
      ...
      {creating && <FormPopup title="Nouveau fournisseur" initial={empty} onSave={handleCreate} onClose={() => setCreating(false)} loading={saving} />}
    </div>
  );
};
