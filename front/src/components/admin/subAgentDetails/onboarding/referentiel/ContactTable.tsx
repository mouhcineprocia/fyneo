...
import { contactApi } from '../../../../../lib/onboarding/contact';
import { Pagination } from './Pagination';

const PAGE_SIZE = 10;
const C = { color: '#8b5cf6', bg: 'rgba(139,92,246,0.1)' };
...
const TYPE_LABELS: Record<string, string> = {
  consultant_interne: 'Consultant interne',
...
const empty: Partial<Contact> = { type: 'other', firstName: '', lastName: '', companyName: '', email: '', phone: '', address: '', city: '', country: 'Maroc', notes: '', status: 'active' };

const FormPopup: React.FC<{ title: string; initial: Partial<Contact>; onSave: (d: Partial<Contact>) => void; onClose: () => void; loading?: boolean }> = ({ title, initial, onSave, onClose, loading }) => {
      ...
      <div style={{ width: '68vw', maxWidth: 820, maxHeight: '90vh', background: 'var(--bg)', borderRadius: 16, boxShadow: '0 32px 80px rgba(0,0,0,0.35)', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'onb-popup 0.2s ease-out' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 20px', marginBottom: 18 }}>
            ...
            <Field label="Entreprise" value={form.companyName || ''} onChange={set('companyName')} />
            <Field label="Type *" value={form.type || ''} onChange={set('type')} options={Object.keys(TYPE_LABELS)} />
          </div>
          <p style={sectionSt}>Coordonnées</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 20px', marginBottom: 18 }}>
...
export const ContactTable: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await contactApi.getAll({ page, limit: PAGE_SIZE, search: search || undefined, status: statusF || undefined, country: countryF || undefined, type: typeF || undefined });
      setData(res.data); setTotal(res.total); setTotalPages(res.totalPages);
    } catch (e: any) { setError(e.message || 'Erreur'); } finally { setLoading(false); }
  }, [page, search, statusF, countryF, typeF]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { setPage(0); }, [search, statusF, countryF, typeF]);

  const handleCreate = async (dto: Partial<Contact>) => { setSaving(true); try { await contactApi.create(dto); setCreating(false); load(); } catch (e: any) { alert(e.message); } finally { setSaving(false); } };
  const handleEdit   = async (dto: Partial<Contact>) => { if (!editing) return; setSaving(true); try { await contactApi.update(editing.id, dto); setEditing(null); load(); } catch (e: any) { alert(e.message); } finally { setSaving(false); } };
  const handleDelete = async (id: string) => { if (!confirm('Supprimer ce contact ?')) return; try { await contactApi.remove(id); load(); } catch (e: any) { alert(e.message); } };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border)', background: 'var(--bg2)', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <select value={typeF} onChange={e => setTypeF(e.target.value)} style={selSt}>
          <option value="">Tous les types</option>
          {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
        {total > 0 && <span style={{ fontSize: '0.7rem', color: C.color, fontWeight: 600 }}>{total} contact{total > 1 ? 's' : ''}</span>}
        <button onClick={() => setCreating(true)} style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, border: `1.5px solid ${C.color}`, background: C.bg, color: C.color, cursor: 'pointer', fontSize: '0.76rem', fontWeight: 700, flexShrink: 0, whiteSpace: 'nowrap' }}>
          <Icons.Plus style={{ width: 13, height: 13 }} /> Nouveau contact
        </button>
      </div>

      <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto' }}>
        {loading ? <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text3)', fontSize: '0.85rem' }}>Chargement…</div>
        : error ? <div style={{ textAlign: 'center', padding: '60px 0', color: '#ef4444', fontSize: '0.85rem' }}>{error}</div>
        : data.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text3)' }}>
            <Icons.Users style={{ width: 32, height: 32, margin: '0 auto 10px', display: 'block', color: 'var(--border)' }} />
            <div style={{ fontSize: '0.85rem' }}>Aucun contact trouvé</div>
          </div>
        ) : (
          <table style={{ width: '100%', minWidth: 820, borderCollapse: 'collapse', fontSize: '0.74rem' }}>
            <thead style={{ position: 'sticky', top: 0, zIndex: 2 }}>
              <tr style={{ background: 'var(--bg2)', borderBottom: '1px solid var(--border)' }}>
                {['Nom', 'Entreprise', 'Email', 'Téléphone', 'Pays / Ville', 'Type', 'Statut', ''].map(h => (
                  ...
                  <tr key={d.id} onClick={() => setDetail(d)}
                    ...
                    <td style={{ padding: '10px 11px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 30, height: 30, borderRadius: '50%', background: C.bg, border: `1px solid ${C.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontWeight: 700, fontSize: 13, color: C.color }}>{fullName.charAt(0)}</div>
                        <div style={{ fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap' }}>{fullName}</div>
                      </div>
                    </td>
                    <td style={{ padding: '10px 11px', color: 'var(--text2)', whiteSpace: 'nowrap', fontSize: '0.71rem' }}>{d.companyName || '—'}</td>
                    ...
                    <td style={{ padding: '10px 11px' }}><span style={{ padding: '2px 7px', borderRadius: 4, fontSize: '0.63rem', fontWeight: 600, background: C.bg, color: C.color, whiteSpace: 'nowrap' }}>{TYPE_LABELS[d.type] || d.type}</span></td>
      ...
      {detail   && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1500, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={e => e.target === e.currentTarget && setDetail(null)}>
          <div style={{ width: '52vw', maxWidth: 620, maxHeight: '84vh', background: 'var(--bg)', borderRadius: 16, boxShadow: '0 32px 80px rgba(0,0,0,0.35)', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'onb-popup 0.2s ease-out' }}>
            <div style={{ padding: '18px 22px', background: `linear-gradient(135deg,${C.bg},transparent)`, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexShrink: 0 }}>
              <div>
                <div style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--text)' }}>{`${detail.firstName || ''} ${detail.lastName || ''}`.trim()}</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text2)', marginTop: 3 }}>{TYPE_LABELS[detail.type]} {detail.companyName && `· ${detail.companyName}`}</div>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '18px 22px' }}>
              {[
                { title: 'Contact', rows: [['Email', detail.email], ['Téléphone', detail.phone], ['Mobile', detail.mobile]] },
      ...
      {editing  && <FormPopup title={`Modifier — ${editing.firstName} ${editing.lastName}`} initial={editing} onSave={handleEdit} onClose={() => setEditing(null)} loading={saving} />}
      {creating && <FormPopup title="Nouveau contact" initial={empty} onSave={handleCreate} onClose={() => setCreating(false)} loading={saving} />}
    </div>
  );
};
