...
const MOCK_CUSTOMERS: Customer[] = [
  { id: 'c1', name: 'Acme Corp' }, { id: 'c2', name: 'Beta Solutions' },
  { id: 'c3', name: 'Gamma Industries' }, { id: 'c4', name: 'Delta Corp' },
  { id: 'c5', name: 'Epsilon SA' }, { id: 'c6', name: 'Zeta Services' },
  { id: 'c7', name: 'Eta Consulting' }, { id: 'c8', name: 'Theta Groupe' },
];

const MOCK_PROJECTS: Project[] = [
  { id: 'p1',  name: 'Transformation Digitale 2024', type: 'regie',   status: 'active',    budget: 120000, startDate: '2024-01-15', endDate: '2024-12-31', description: "Modernisation des systèmes d'information", customerId: 'c1' },
  { id: 'p2',  name: 'Déploiement ERP',               type: 'regie',   status: 'active',    budget: 85000,  startDate: '2024-02-01', endDate: '2024-09-30', description: "Mise en place d'un ERP sur mesure",        customerId: 'c2' },
  { id: 'p3',  name: 'Audit Sécurité SI',             type: 'regie',   status: 'completed', budget: 34000,  startDate: '2023-10-01', endDate: '2024-01-31', description: 'Audit complet de la sécurité informatique',  customerId: 'c3' },
  { id: 'p4',  name: 'Migration Cloud',               type: 'regie',   status: 'pending',   budget: 67000,  startDate: '2024-04-01', endDate: '2024-10-31', description: 'Migration vers AWS',                         customerId: 'c4' },
  { id: 'p5',  name: 'Support Technique Annuel',      type: 'regie',   status: 'active',    budget: 48000,  startDate: '2024-01-01', endDate: '2024-12-31', description: 'Contrat de maintenance et support',          customerId: 'c5' },
  { id: 'p6',  name: 'Refonte Site Web',              type: 'regie',   status: 'cancelled', budget: 22000,  startDate: '2024-03-01', endDate: '2024-06-30', description: 'Refonte complète du site corporate',         customerId: 'c6' },
  { id: 'p7',  name: 'Formation DevOps',              type: 'regie',   status: 'completed', budget: 15000,  startDate: '2023-11-01', endDate: '2024-02-28', description: 'Programme de formation DevOps',              customerId: 'c7' },
  { id: 'p8',  name: 'Conseil Stratégique Q2',        type: 'regie',   status: 'pending',   budget: 30000,  startDate: '2024-04-01', endDate: '2024-06-30', description: 'Mission de conseil stratégique',             customerId: 'c8' },
  { id: 'p9',  name: 'Revente Licences Microsoft',   type: 'negoce',  status: 'active',    budget: 54000,  startDate: '2024-01-10', endDate: '2024-12-31', description: 'Distribution de licences Microsoft 365',    customerId: 'c1' },
  { id: 'p10', name: 'Infrastructure Réseau',         type: 'negoce',  status: 'completed', budget: 91000,  startDate: '2023-09-01', endDate: '2024-03-31', description: 'Fourniture et installation réseau',          customerId: 'c3' },
  { id: 'p11', name: 'Matériel Informatique Batch',  type: 'negoce',  status: 'active',    budget: 76000,  startDate: '2024-02-15', endDate: '2024-08-31', description: 'Achat-revente matériel informatique',        customerId: 'c4' },
  { id: 'p12', name: 'Solutions Stockage Cloud',      type: 'negoce',  status: 'pending',   budget: 43000,  startDate: '2024-05-01', endDate: '2025-04-30', description: 'Contrat stockage cloud enterprise',          customerId: 'c2' },
  { id: 'p13', name: 'Serveurs Data Center',          type: 'negoce',  status: 'active',    budget: 138000, startDate: '2024-01-20', endDate: '2025-01-19', description: 'Fourniture serveurs haute performance',      customerId: 'c6' },
  { id: 'p14', name: 'Équipements Sécurité',          type: 'negoce',  status: 'cancelled', budget: 27000,  startDate: '2024-03-15', endDate: '2024-07-15', description: "Caméras et contrôle d'accès",               customerId: 'c5' },
  { id: 'p15', name: 'Licences Antivirus Entreprise', type: 'negoce',  status: 'completed', budget: 18500,  startDate: '2023-12-01', endDate: '2024-11-30', description: 'Licences antivirus pour 500 postes',         customerId: 'c7' },
  { id: 'p16', name: 'Téléphonie IP',                 type: 'negoce',  status: 'pending',   budget: 62000,  startDate: '2024-06-01', endDate: '2025-05-31', description: 'Déploiement téléphonie VoIP',                customerId: 'c8' },
];
                ...
                {group.map(al => (
                  <div key={al.id} style={{ borderRadius: 12, overflow: 'hidden', background: m.bg, border: `1px solid ${m.border}`, boxShadow: '0 1px 8px rgba(0,0,0,0.04)', animation: 'pr-fadein 0.3s ease' }}>
                    ...
                    <div style={{ padding: '8px 12px', display: 'flex', alignItems: 'flex-end', gap: 8 }}>
                      ...
                      {al.project && (
                        <span style={{ flexShrink: 0, fontSize: '0.56rem', fontWeight: 600, padding: '2px 7px', borderRadius: 6, background: 'var(--bg)', color: m.color, border: `1px solid ${m.border}` }}>{al.project}</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ ...glass('rgba(139,92,246,0.2)'), padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 8 }}>
              {[
                { label: 'Régie',  val: regieBudget,  color: '#0d9394', pct: Math.round((regieBudget / totalBudget) * 100) },
                { label: 'Négoce', val: negoceBudget, color: '#8b5cf6', pct: Math.round((negoceBudget / totalBudget) * 100) },
              ].map(b => (
...
// ─── Details View (existing project table) ────────────────────────────────────
...
const DetailsView: React.FC<DetailsViewProps> = ({ setSelectedProject, setDetailsOpen }) => {
  ...
  const SOFIA_FULL = "Salut! Je suis Sofia, ton agent IA. Je gère 16 projets : 8 en Régie (421 000 €) et 8 en Négoce (509 500 €). 4 projets sont en attente de validation.";
  const [sofiaText, setSofiaText] = useState('');
  const [sofiaThinking, setSofiaThinking] = useState(true);

  useEffect(() => {
    const t = setTimeout(() => { setSofiaThinking(false); setIsLoading(false); }, 700);
  ...
  useEffect(() => { setPage(0); }, [searchQuery, activeTab, selectedStatus, selectedClient]);

  const baseFiltered = useMemo(() => MOCK_PROJECTS.filter(p => {
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      const cust = MOCK_CUSTOMERS.find(c => c.id === p.customerId);
      if (!p.name.toLowerCase().includes(q) && !(cust?.name || '').toLowerCase().includes(q)) return false;
    }
    if (selectedStatus && p.status !== selectedStatus) return false;
    if (selectedClient && p.customerId !== selectedClient) return false;
    if (dateDebut && p.startDate < dateDebut) return false;
    if (dateFin && p.startDate > dateFin) return false;
    return true;
  }), [searchQuery, selectedStatus, selectedClient, dateDebut, dateFin]);

  const filteredProjects = useMemo(() => activeTab === 'all' ? baseFiltered : baseFiltered.filter(p => p.type === activeTab), [baseFiltered, activeTab]);
  const regieCount  = useMemo(() => baseFiltered.filter(p => p.type === 'regie').length, [baseFiltered]);
  const negoceCount = useMemo(() => baseFiltered.filter(p => p.type === 'negoce').length, [baseFiltered]);
  const tableData   = filteredProjects.slice(page * rowsPerPage, (page + 1) * rowsPerPage);
    ...
    <div style={{ padding: 20, overflowY: 'auto', height: '100%', animation: 'pr-fadein 0.3s ease' }}>
      ...
      <div style={{ background: 'var(--bg)', borderRadius: 10, border: '1px solid var(--border)', padding: '12px 14px', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <div style={{ width: 1, height: 20, background: 'var(--border)' }} />
          <select value={selectedClient} onChange={e => setSelectedClient(e.target.value)} style={{ padding: '4px 8px', fontSize: '0.65rem', border: '1px solid var(--border)', borderRadius: 6, background: 'var(--bg)', color: 'var(--text)', outline: 'none', cursor: 'pointer' }}>
            <option value="">Tous les clients</option>
            {MOCK_CUSTOMERS.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.6rem', color: 'var(--text3)' }}>Date début — De:</span>
          <input type="date" value={dateDebut} onChange={e => setDateDebut(e.target.value)} style={{ ...inputSt, minWidth: 120 }} />
          <span style={{ fontSize: '0.6rem', color: 'var(--text3)' }}>À:</span>
          <input type="date" value={dateFin} onChange={e => setDateFin(e.target.value)} style={{ ...inputSt, minWidth: 120 }} />
          {(searchQuery || selectedStatus || selectedClient || dateDebut || dateFin) && (
            <button onClick={() => { setSearchQuery(''); setSelectedStatus(null); setSelectedClient(''); setDateDebut(''); setDateFin(''); }} style={{ fontSize: '0.62rem', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', padding: '3px 6px' }}>
              Réinitialiser
            </button>
          )}
        </div>
      ...
      <div style={{ background: 'var(--bg)', borderRadius: 10, border: '1px solid var(--border)', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg2)' }}>
          {([
            { id: 'all'    as const, label: 'Tous',   desc: 'Régie + Négoce', count: baseFiltered.length },
            { id: 'regie'  as const, label: 'Régie',  desc: 'Commission',     count: regieCount },
            { id: 'negoce' as const, label: 'Négoce', desc: 'Achat-Revente',  count: negoceCount },
          ]).map(tab => (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem' }}>
            <thead>
              <tr style={{ background: 'var(--bg2)', borderBottom: '1px solid var(--border)' }}>
                {['Projet', 'Type', 'Client', 'Budget', 'Statut', 'Date début', 'Actions'].map((h, i) => (
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={7} style={{ padding: 20, textAlign: 'center', color: 'var(--text2)', fontSize: '0.72rem' }}>Aucun projet à afficher.</td></tr>
              ) : tableData.map(row => {
                ...
                return (
                  <tr key={row.id} onClick={() => { setSelectedProject(row); setDetailsOpen(true); }} style={{ borderBottom: '1px solid var(--bg2)', cursor: 'pointer' }}
                    ...
                    <td style={{ padding: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <div>
                          ...
                          <p style={{ margin: 0, fontSize: '0.58rem', color: 'var(--text3)', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.description}</p>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: 12 }}>
                    ...
                    <td style={{ padding: 12 }}><span style={{ fontSize: '0.68rem', color: 'var(--text2)' }}>{customer?.name || '—'}</span></td>
                    <td style={{ padding: 12 }}>
                    ...
                    <td style={{ padding: 12 }}>
                      <span style={{ fontSize: '0.68rem', color: 'var(--text2)' }}>
                        {new Date(row.startDate).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                      </span>
                    </td>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderTop: '1px solid var(--border)', background: 'var(--bg2)' }}>
          ...
          <span style={{ fontSize: '0.62rem', color: 'var(--text2)' }}>
            {totalCount === 0 ? '0' : `${page * rowsPerPage + 1}–${Math.min((page + 1) * rowsPerPage, totalCount)}`} sur {totalCount}
          </span>
          <div style={{ display: 'flex', gap: 4 }}>
            ...
            <button onClick={() => setPage(p => p + 1)} disabled={(page + 1) * rowsPerPage >= totalCount} style={{ width: 24, height: 24, borderRadius: 6, background: 'var(--bg)', border: '1px solid var(--border)', cursor: (page + 1) * rowsPerPage >= totalCount ? 'default' : 'pointer', opacity: (page + 1) * rowsPerPage >= totalCount ? 0.4 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export const ProjectChatModal: React.FC<ProjectModalProps> = ({ open, agent, agentId, onClose }) => {
  ...
  const NAV = [
    ...
    { key: 'details' as const, label: 'Référentiel',             icon: '☰',  badge: MOCK_PROJECTS.length },
  ];
